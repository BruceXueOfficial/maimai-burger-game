...
function resize(){const width=document.documentElement?.clientWidth||innerWidth,height=document.documentElement?.clientHeight||innerHeight;scale=Math.min(width/1600,height/900);stageWidth=Math.max(1600,width/scale);cv.width=Math.round(stageWidth);handCv.width=Math.round(stageWidth);$('game').style.width=stageWidth+'px';$('game').style.transform=`scale(${scale})`;$('game').style.left='0px';$('game').style.top=(height-900*scale)/2+'px';refreshIngredientZones();}
...
function land(){const a=fall,f=FOOD[a.type],neat=a.landed?R.alignment(a.x,a.previousX):0;const correct=a.type===currentOrder().layers[rows.length];rows.push({type:a.type,landed:a.landed,alignment:neat,x:Math.round(a.x),height:Math.round(a.target-a.from),seconds:Math.round(elapsed*10)/10});if(a.landed){stack.push({type:a.type,x:a.x,y:surface-f.h*.72});surface-=f.lift;previousX=a.x;}
 feedback=!a.landed?'滑落了，这一层未落稳':!correct?'放错了！':neat>=95?'完美贴合！':neat>=75?'不错，继续出餐！':'有点歪啦，下层稳一点';feedbackUntil=elapsed+1.2;beep(!correct||!a.landed?220:neat>=95?950:650,.14);fall=null;selection=null;settle=.2;recipeUI();updateControls();if(serveRequested)finish();else if(rows.length>=12)toast('操作台已满，请点击出餐');}
