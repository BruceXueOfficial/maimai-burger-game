...
function recipeUI(){ $('orderName').textContent=currentOrder().name;$('orderProgress').textContent=`第 ${orderIndex+1} / 10 单`;$('orderFoot').textContent=`底包已放好 · 配方 ${currentOrder().layers.length} 层 · 游戏简化版`;$('speedTarget').textContent=`${currentOrder().layers.length*3} 秒内完成，速度满分`;$('queue').innerHTML=R.ORDERS.map((o,i)=>`<i class="${i<orderIndex?'done':i===orderIndex?'current':''}"></i>`).join('');$('recipe').innerHTML=currentOrder().layers.map((t,i)=>`<li class="${rows[i]?(rows[i].landed&&rows[i].type===t?'good':'bad'):i===rows.length?'current':''}"><span class="number">${i+1}</span>${FOOD[t].name}<em>${rows[i]?(rows[i].landed&&rows[i].type===t?'✓':'×'):i===rows.length?'下一层':''}</em></li>`).join('');}
